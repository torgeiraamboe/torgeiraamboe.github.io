...
We also know that $MU$ carries the universal formal group law $G$ over its coefficient ring, the Lazard ring $L$. What happens then when we $p$-localize? The coefficient ring becomes $L\otimes \mathbb{Z}_{(p)}$ and we can ask whether we again get some sort of universal formal group law, now not for all rings but for all $\mathbb{Z}_{(p)}$-algebras. To reiterate the question formally: Is there a formal group law over $L\otimes \mathbb{Z}_{(p)}$ that is universal among $\mathbb{Z}_{(p)}$-algebras? The answer is yes, and it is simply the image $G_{(p)}$ of the universal group law $G$ under the canonical map $L\longrightarrow L\otimes \mathbb{Z}_{(p)}$. Thus also orientations on $p$-local spectra are classified by multiplicative maps $MU_{(p)}\longrightarrow E$. 

We are now going to do something that might seem very unmotivated, due to the fact that there are some tricky mathematics lying in the background that we will not cover in this post. For the interested this is called $p$-typical formal group laws. My intuition behind them is that they are nice formal group laws over $\mathbb{Z}_{(p)}$-algebras that have generators in exponential periodic degrees. We will see what we mean by this soon. 

Ok, take the formal group law $G_{(p)}$ over the ring $L\otimes \mathbb{Z}_{(p)}$ as described above. Recall that the Lazard ring $L$ is the ring $\mathbb{Z}[b_1, b_2, \ldots]$ where $b_i$ has degree $2i$. We construct an endomorphism $\psi:L\otimes \mathbb{Z}_{(p)} \longrightarrow L\otimes \mathbb{Z}_{(p)}$ by $\psi(b_i) = b_i$ if $i=p^k-1$ for some $k$, and $\psi(b_i)=0$ otherwise. The image of $\psi$ is a subring $V\subset L\otimes \mathbb{Z}_{(p)}$, more specifically it is the subring $\mathbb{Z}_{(p)}[v_1, v_2, \ldots ]$ where $v_i$ has degree $2(p^i-1)$. This is the periodicity in degrees we mentioned above. This ring carries a formal group law $G^p$ that is universal among $p$-typical formal group laws. This means that among $p$-typical formal group laws, $V$ acts like the Lazard ring does for all formal group laws. Because of this $V$ is often denoted $L^p$ and called the $p$-typical Lazard ring.

A question we can ask is then: Is being $p$-typical a strict property? It turns out that it is not. In fact, by a theorem of Cartier, all formal group laws over a $\mathbb{Z}_{(p)}$-algebra is strictly isomorphic to a $p$-typical formal group law. Hence using the ring $V$ and the universal $p$-typical formal group laws gives us some advantages over using $L\otimes \mathbb{Z}_{(p)}$ and $G_{(p)}$ as we now have nice periodic behavior and a nicely defined ring more similar to the Lazard ring. We don’t really understand yet why this periodic behavior is important, but we will see this in the future. Intuitively, if we choose some arbitrary even degree, then $L$ has incredibly many elements in that degree. Using $V$ allows us to pick out only one of these in an orderly exponential manner. This has the benefits of reducing the overall complexity of the theory immensely. Almost all cohomology theories we will work with in the future has some kind of periodic property, an it all stems from the ones just described. 
...
Since $e$ is an idempotent, $e^\ast(pt)$ will also be one. Luckily for us we have already seen this map, and it is in fact the map $\psi$ used to create the $p$-typical Lazard ring $V$ from $L\otimes \mathbb{Z}_{(p)}$. As the coefficient ring of $BP$ is the image of $e^\ast(pt)$, i.e. the image of $\psi$, we have 

$$
BP^\ast(pt) \cong V \cong \mathbb{Z}_{(p)}[v_1, v_2, \ldots]
$$
